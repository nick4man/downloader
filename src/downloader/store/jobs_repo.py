...
_COLUMNS = (
    ...
    "bytes_done, bytes_total, sha256, error, position, audio, source_id, height, "
...
async def add_job(conn: aiosqlite.Connection, job: DownloadJob) -> None:
    ...
    await conn.execute(
        ...
        ":source_id, :height, :created_at, :updated_at)",
        _job_params(job),
    )
    await conn.commit()
...
async def update_job(conn: aiosqlite.Connection, job: DownloadJob) -> None:
    ...
    await conn.execute(
        ...
        "sha256=:sha256, error=:error, position=:position, "
        "source_id=:source_id, height=:height, updated_at=:updated_at WHERE id=:id",
        _job_params(job),
    )
    await conn.commit()
